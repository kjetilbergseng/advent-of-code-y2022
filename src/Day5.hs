...
prependCreates fn pos createsToPrepend creates = take (pos-1) creates ++ [fn createsToPrepend ++ creates !! (pos-1)]  ++ drop pos creates

dropCrates :: Int -> Int -> [[a]] -> [[a]]
dropCrates number pos creates = take (pos-1) creates ++ [drop number (creates !! (pos-1))]  ++ drop pos creates
